...
def generate_civil_metadata():
    csv_path = "data/civil_master.csv"
    parquet_path = "data/civil_master.parquet"
    audit_path = "reports/civil_master_audit.csv"
    report_path = "reports/civil_master_metadata.md"

    print("--- Running Civil Master Dataset Metadata & Quality Audit ---", flush=True)
    ...
    domain_valid = (set(df["domain"].unique()) == {"Civil"})
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Civil Engineering Master Dataset Metadata & Quality Audit Report\n\n")
        ...
        f.write("|---|---|---|---|\n")
        f.write(f"| Column Schema | `['text', 'role', 'domain', 'source', 'source_job_id']` | `{list(df.columns[:5])}` | {'PASS' if schema_valid else 'FAIL'} |\n")
        ...
        f.write(f"| Domain Uniformity | `domain == 'Civil'` for 100% records | {domain_counts} | {'PASS' if domain_valid else 'FAIL'} |\n")
    ...
    print(f"Report generated successfully at: {report_path}", flush=True)

if __name__ == "__main__":
    generate_civil_metadata()
